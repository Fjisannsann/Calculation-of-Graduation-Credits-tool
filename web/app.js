...
function parseRowsFromLines(lines, subjectMetaMap) {
  ...
  const numbersFirstPattern =
    /(\d+(?:\.\d+)?)\s+([0-5])\s+(\d{2})\s*([^0-9].*?)(?=(?:\d+(?:\.\d+)?\s+[0-5]\s+\d{2}\s*[^0-9])|$)/g;
  const subjectFirstPattern =
    /([^0-9].*?)\s+(\d+(?:\.\d+)?)\s+([0-5])\s+(\d{2})(?=\s+[^0-9].*?\s+\d+(?:\.\d+)?\s+[0-5]\s+\d{2}|$)/g;

  const pushRow = (credits, grade, year, subjectRaw) => {
    const subjectClean = normalizeText(subjectRaw)
      .replace(/\d+(?:\.\d+)?\s*小計.*$/, "")
      .replace(/(学期GPA|年度GPA|累積GPA).*/, "")
      .trim();
    if (!subjectClean) return;

    const subjectParsed = normalizeSubjectName(subjectClean);
    if (!subjectParsed) return;

    const uniqueKey = `${credits}|${grade}|${year}|${subjectParsed}`;
  ...
  for (const rawLine of lines) {
    ...
    let matchedCount = 0;
    numbersFirstPattern.lastIndex = 0;
    for (const matched of line.matchAll(numbersFirstPattern)) {
      matchedCount += 1;
      pushRow(matched[1], matched[2], matched[3], matched[4]);
    }

    if (matchedCount > 0) continue;

    subjectFirstPattern.lastIndex = 0;
    for (const matched of line.matchAll(subjectFirstPattern)) {
      pushRow(matched[2], matched[3], matched[4], matched[1]);
    }
  }
...
runButton.addEventListener("click", async () => {
  ...
  try {
    ...
    for (const file of files) {
      ...
      if (rows.length === 0) {
        throw new Error(`科目行を抽出できませんでした: ${file.name}`);
      }
